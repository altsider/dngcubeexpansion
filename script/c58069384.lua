...
function s.initial_effect(c)
	...
	c:EnableReviveLimit()
	--Special Summon a "Cyber Dragon" from GY
	...
	c:RegisterEffect(e2)
	--Special Summon a Machine Fusion when sent to GY
...
function s.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsFaceup() and c:IsRelateToEffect(e) then
		--Gain 2100 ATK until the end of this turn
		local e1=Effect.CreateEffect(c)
...
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	return rp==1-tp and (r&REASON_EFFECT)~=0 and e:GetHandler():IsPreviousControler(tp)
end
function s.spfilter(c,e,tp)
	return c:IsType(TYPE_FUSION) and c:IsRace(RACE_MACHINE) and Duel.GetLocationCountFromEx(tp,tp,nil,c)>0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
