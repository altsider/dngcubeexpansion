...
s.listed_names={1160884672}

function s.fcfilter(c)
	return c:IsCode(1160884672) and c:IsAbleToHand()
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.fcfilter,tp,LOCATION_DECK,0,1,nil)
			or (Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,1160884672),tp,LOCATION_FZONE,0,1,nil) 
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local b=Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,1160884672),tp,LOCATION_FZONE,0,1,nil) 
		and Duel.IsExistingMatchingCard(s.mcfilter,tp,LOCATION_DECK,0,1,nil,tp)
