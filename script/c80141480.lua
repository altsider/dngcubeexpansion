...
function script.initial_effect(card)
	...
	atk_effect:SetProperty(EFFECT_FLAG_SINGLE_RANGE)
...
function script.apply_change_atk(effect, card)
	...
	local inferior 	= INFERIOR_ATTRIBUTE[attribute]
...
function script.limit_attribute(effect, card)
	...
	local superior 	= SUPERIOR_ATTRIBUTE[attribute]

	return card and card:IsAttribute(superior)
end