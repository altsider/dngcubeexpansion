...
function s.initial_effect(c)
	--synchro summon
	Synchro.AddProcedure(c,aux.FilterSummonCode(1168505803),1,1,Synchro.NonTuner(nil),1,99)
	c:EnableReviveLimit()
...
s.material={1168505803}
s.listed_names={1168505803}
function s.valcheck(e,c)
	local g=c:GetMaterial()
	local att=0
	local tc=g:GetFirst()
	for tc in aux.Next(g) do
		if not tc:IsCode(1168505803) or not tc:IsType(TYPE_TUNER) then
